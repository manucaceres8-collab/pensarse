...
export default function MiHome() {
  ...
  async function loadPatient() {
    const res = await fetch("/api/panel-demo/patients/me", { cache: "no-store" });
    if (!res.ok) return;
  ...
  async function guardarCheckin() {
    ...
    try {
      const res = await fetch("/api/panel-demo/patients/me/checkins", {
        method: "POST",
  ...
  async function guardarNota() {
    ...
    setSavingNote(true);
    try {
      await fetch("/api/panel-demo/patients/me/notes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <section className="rounded-[26px] border border-[#d7deea] bg-white p-6">
        ...
        <div className="mt-4 space-y-3">
          ...
          {taskPreview.map((task) => (
            <Link
              key={task.id}
              href={`/demo/paciente/tareas/${task.id}`}
