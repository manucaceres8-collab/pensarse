...
export default function PacientesPage() {
  ...
  useEffect(() => {
    ...
    async function loadPatients() {
      try {
        const res = await fetch("/api/panel-demo/patients", { cache: "no-store" });
  ...
  async function removePatient(patientId: string, patientName: string) {
    ...
    try {
      const res = await fetch(`/api/panel-demo/patients/${patientId}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Link
          href="/panel-demo/pacientes/nuevo"
            ...
            return (
              <div key={patient.id} className="rounded-[24px] border border-[#d7deea] bg-white p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <Link
                    href={`/panel-demo/pacientes/${patient.id}`}
