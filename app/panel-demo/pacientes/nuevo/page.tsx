...
export default function NuevoPacientePage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!email.trim() || password.trim().length < 6) {
      setError("Email y contraseña del paciente son obligatorios (mínimo 6 caracteres).");
      return;
    }

    setSaving(true);
    try {
      const res = await fetch("/api/panel-demo/patients", {
        method: "POST",
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-[26px] border border-[#d7deea] bg-white p-6">
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="mb-1 block text-sm text-[#607794]">Contraseña inicial del paciente</label>
            <input
              type="password"
              value={password}
