...
export default function TareaPacienteDetallePage() {
    ...
    async function load() {
      ...
      try {
        const [patientRes, templatesRes] = await Promise.all([
          fetch(`/api/panel-demo/patients/${patientId}`, { cache: "no-store" }),
  ...
  if (!patient || !task) {
    return (
      <div className="space-y-4">
        <Link href={`/panel-demo/pacientes/${patientId}`} className="text-sm text-[#607794] hover:text-[#1f304b]">
  ...
  return (
    <div className="space-y-6">
      <Link href={`/panel-demo/pacientes/${patient.id}`} className="text-sm text-[#607794] hover:text-[#1f304b]">
        Volver a ficha del paciente
      </Link>
